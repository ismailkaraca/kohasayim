...
const transformReportData = (data) => {
    ...
    return data.map(originalItem => {
        ...
        const transformedItem = { ...item, 'Materyal Statüsü': MATERIAL_STATUS_MAP[item['materyal_statusu_kodu']] || item['materyal_statusu_kodu'], 'Ödünç Verilebilirlik': LOAN_ELIGIBILITY_MAP[item['odunc_verilebilirlik_kodu']] || item['odunc_verilebilirlik_kodu'], 'Materyalin Yeri': LOCATION_CODE_MAP[item['materyalin_yeri_kodu']] || item['materyalin_yeri_kodu'], };
                        ...
                        {filteredScannedItems.slice(0, visibleItemsCount).map((item, index) => {
                             ...
                             return (
                                <div key={`${item.timestamp}-${index}`} className={`p-2 rounded-md border flex items-center justify-between gap-2 ${item.isValid ? 'bg-white' : 'bg-yellow-50'}`}>
                                    <div className="flex-grow"><p className="font-mono text-slate-800">{item.barcode}</p><p className="text-xs text-slate-600">{itemData?.['eser_adi'] || 'Bilinmeyen Eser'}</p></div>
...
export default function App() {
    ...
    const PRE_ANALYSIS_REPORTS_CONFIG = useMemo(() => [ { id: 'preOnLoan', title: 'Ödünçteki Materyaller', format: '.xlsx', icon: ICONS.onLoan, description: 'Koha verisine göre halihazırda bir okuyucunun üzerinde ödünçte görünen materyaller.', generator: () => { const data = kohaData.filter(i => String(i['odunc_durumu']) === '1'); downloadXlsx(transformReportData(data), `on_analiz_oduncteki_materyaller_${currentSessionName}.xlsx`); } }, { id: 'preStatusIssues', title: 'Düşüm / Devir Statüsündeki Materyaller', format: '.xlsx', icon: ICONS.status, description: 'Koha verisine göre materyal statüsü "düşüm" veya "devir" gibi koleksiyon dışı bir durumu gösteren tüm materyaller.', generator: () => { const data = kohaData.filter(i => String(i['materyal_statusu_kodu']) !== '0'); downloadXlsx(transformReportData(data), `on_analiz_dusum_devir_statulu_${currentSessionName}.xlsx`); } }, { id: 'preNotLoanable', title: 'Ödünç Verilebilirlik Durumu "Ödünç Verilemez" Olan Materyaller', format: '.xlsx', icon: ICONS.notLoanable, description: 'Koha verisine göre Ödünç Verilebilirlik Durumu "Ödünç Verilebilir" Olmayan Tüm Materyaller.', generator: () => { const data = kohaData.filter(i => String(i['odunc_verilebilirlik_kodu']) !== '0'); downloadXlsx(transformReportData(data), `on_analiz_odunc_verilemeyenler_${currentSessionName}.xlsx`); } }, ], [kohaData, currentSessionName, isXlsxReady]);
